...
public class SolutionPart2 {
    ...
    public static void main(String[] args) throws FileNotFoundException{
        ...
        final int MAX_SPACE = 70000000;
        final int UPDATE_SIZE = 30000000;
        int availableSpace = MAX_SPACE - UPDATE_SIZE;
        int answer = 0;
        ...
        availableSpace -= directories.get(0).getSize();
        answer = directories.get(0).getSize();

        for (Directory d:directories){
            if (d.getName().equals("/")){
                continue;
            }
            if (d.getSize() >= availableSpace * -1){
                if (d.getSize() < answer){
                    answer = d.getSize();
                }
            }
